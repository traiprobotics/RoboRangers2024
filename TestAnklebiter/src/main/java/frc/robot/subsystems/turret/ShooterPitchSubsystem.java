...
public class ShooterPitchSubsystem extends SubsystemBase {
  ...
  public void updateDashboard() {
    ...
    if((p != PIDConstants.PITCH_P)) { 
      shooterPIDController.setP(p);
      PIDConstants.PITCH_P = p;
    }
    if((i != PIDConstants.PITCH_I)) {
      shooterPIDController.setI(i);
      PIDConstants.PITCH_I = i;
    }
    if((d != PIDConstants.PITCH_D)) { 
      shooterPIDController.setD(d);
      PIDConstants.PITCH_D = d;
    }
    if((ff != PIDConstants.PITCH_FF)) {
      shooterPIDController.setFF(ff);
      PIDConstants.PITCH_FF = ff;
    }
